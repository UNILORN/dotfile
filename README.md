...
```sh
./scripts/macos/apply-fish-config.sh
```
...
macOS では Homebrew 前提の番号トグル式メニューを開き、以下をまとめてインストールできます。
...
- `brew` が未導入の場合は処理を中断し、先に `https://brew.sh/` でのセットアップを促します。
- `Docker Desktop` はインストール後に GUI から初回起動が必要です。
- `git` は通常 Command Line Tools に含まれます。未導入なら `xcode-select --install` を実行してください。
